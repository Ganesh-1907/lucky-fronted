...
function GrowthBadge({ value }: { value: number }) {
  ...
  return (
    <span className={cn(
      ...
      isPositive ? "bg-violet-50 text-violet-600" : "bg-red-500/15 text-red-400"
    )}>
...
export default function CustomerAnalyticsPage() {
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {Array.from({ length: 8 }).map((_, i) => <div key={i} className="h-32 bg-white rounded-2xl animate-pulse" />)}
        </div>
      </div>
    );
  }
  ...
  const kpiCards = [
    ...
    { label: "Active Customers", value: data?.activeCustomers?.toLocaleString() || "0", icon: <UserCheck size={18} />, gradient: "from-emerald-500 to-green-600", glow: "shadow-violet-500/25" },
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-gray-900" style={{ fontFamily: "var(--font-outfit)" }}>Customer Analytics</h1>
        <p className="text-sm text-gray-500 mt-1">Understand customer acquisition, retention, and lifetime value.</p>
      </div>

      {/* KPI Cards */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {kpiCards.map((kpi, i) => (
          <div key={i} className="bg-white border border-gray-100 rounded-2xl p-5 hover:border-gray-600/60 transition-all">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5">
          <h3 className="text-sm font-bold text-gray-900 mb-5 flex items-center gap-2" style={{ fontFamily: "var(--font-outfit)" }}>
            <BarChart3 size={14} className="text-violet-600" /> Customer Growth (Last 6 Months)
          </h3>
          {growthData.length === 0 ? (
              ...
              {growthData.map((g: any, i: number) => {
                ...
                return (
                  <div key={i} className="flex flex-col items-center gap-2 flex-1">
                    <span className="text-[10px] text-gray-500 font-bold">{g.new_customers}</span>
          ...
          <div className="space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-bold text-gray-900">{data?.retentionRate || 0}%</span>
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-bold text-gray-900">{data?.repeatRate || 0}%</span>
              </div>
              <div className="h-3 bg-gray-700/40 rounded-full overflow-hidden">
            ...
            <div className="pt-2 border-t border-gray-700/30">
              ...
              <div className="flex justify-between mt-2">
                <span className="text-[10px] text-violet-600 flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-cyan-500" /> New</span>
