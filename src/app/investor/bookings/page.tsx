...
const STATUS_CONFIGS: Record<string, { icon: React.ReactNode; gradient: string; glow: string }> = {
  ...
  COMPLETED: { icon: <CheckCircle2 size={18} />, gradient: "from-emerald-500 to-green-600", glow: "shadow-violet-500/25" },
...
export default function BookingAnalyticsPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-gray-900" style={{ fontFamily: "var(--font-outfit)" }}>Booking Analytics</h1>
        <p className="text-sm text-gray-500 mt-1">Analyze booking patterns, status distribution, and growth trends.</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        {statusData.map((s: any) => {
          ...
          return (
            <div key={s.status} className="bg-white border border-gray-100 rounded-2xl p-4 hover:border-gray-600/60 transition-all">
              <div className={cn("w-9 h-9 rounded-xl bg-gradient-to-br flex items-center justify-center text-gray-900 mb-2 shadow-lg", cfg.gradient, cfg.glow)}>
                {cfg.icon}
              </div>
              <p className="text-xl font-bold text-gray-900" style={{ fontFamily: "var(--font-outfit)" }}>{s.count}</p>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-4 hover:border-gray-600/60 transition-all">
          <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-white/10 to-white/5 flex items-center justify-center text-gray-900 mb-2">
            <ShoppingBag size={18} />
          </div>
          <p className="text-xl font-bold text-gray-900" style={{ fontFamily: "var(--font-outfit)" }}>{data?.total || 0}</p>
            ...
            {growthData.map((g: any, i: number) => {
              ...
              return (
                <div key={i} className="flex flex-col items-center gap-1.5 flex-1">
                  <span className="text-[9px] text-gray-500 font-bold">{g.bookings}</span>
                ...
                <tbody>
                  {cityData.map((c: any, i: number) => (
                    <tr key={i} className="border-b border-gray-700/15">
                      ...
                      <td className="p-2 text-sm text-gray-900 text-right font-semibold">{c.bookings}</td>
                      <td className="p-2 text-sm text-violet-600 text-right font-semibold">{formatPrice(c.revenue)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
